...
from data.config import (
    ...
    FRUIT_SCALES, TEXTURE_ATLAS_PATH, BOMB_MODEL, EASY_SPEED
)


class EasyGameScene(Scene):
    def __init__(self, manager):
        ...
        # Musique de fond (Easy Mode)
        self.ost_path = "asset/Sound Menu/easy_background_ost.ogg"
    ...
    def reset(self):
        ...
        self.highscore_triggered = False 
    ...
    def on_enter(self):
        ...
        try:
            ...
            print("Erreur : Impossible de lire easy_background_ost.ogg")
    ...
    def update(self, dt):
        ...
        if self.lives <= 0:
            if not self.game_over:
                self.game_over = True
                pygame.mixer.music.stop()
                # Transmission identique au mode Normal : score et mode
                self.manager.shared_data['score'] = self.score.current
                self.manager.shared_data['mode'] = 'easy'
        ...
        effective_dt = dt * time_mult * EASY_SPEED

        self.spawn_timer += effective_dt
        spawn_interval = get_spawn_interval(self.score.current) * 1.2
    ...
    def render_overlay(self, surface):
        ...
        if self.paused:
            ...
            surface.blit(overlay, (0, 0))
            pause_txt = self.pause_font.render("PAUSE - EASY", True, (255, 255, 255))
    ...
    def spawn_objects(self):
        count = 3 if (self.frenzy_timer > 0 or self.freeze_timer > 0) else 2
        for _ in range(count):
            x = random.randint(100, SCREEN_WIDTH - 100)
            rand_val = random.random()
            if rand_val < (BOMB_CHANCE * 0.5):
                ...
                if random.random() < 0.15: fruit.fruit_type = random.choice(['ice', 'chili'])
