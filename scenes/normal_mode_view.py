...
class NormalGameScene(Scene):
    def __init__(self, manager):
        ...
        # Musique de fond (Normal Mode)
        self.ost_path = "asset/Sound Menu/normal_background_ost.ogg"
    ...
    def on_enter(self):
        ...
        try:
            ...
            # Démarrage à 5 secondes
            pygame.mixer.music.play(-1, start=5.0)
        except:
            print("Erreur : Impossible de lire normal_background_ost.ogg")
    ...
    def update(self, dt):
        ...
        if self.lives <= 0:
            if not self.game_over:
                self.game_over = True
                pygame.mixer.music.stop()
                # Transmission du score final ET DU MODE avant changement de scène
                self.manager.shared_data['score'] = self.score.current
                self.manager.shared_data['mode'] = 'normal'
    ...
    def render_overlay(self, surface):
        ...
        if self.paused:
            ...
            surface.blit(overlay, (0, 0))
            pause_txt = self.pause_font.render("PAUSE - NORMAL", True, (255, 255, 255))
