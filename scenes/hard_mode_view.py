...
from data.config import (
    ...
    FRUIT_SCALES, TEXTURE_ATLAS_PATH, BOMB_MODEL, HARD_SPEED
)


class HardGameScene(Scene):
    def __init__(self, manager):
        ...
        # Musique de fond (Hard Mode)
        self.ost_path = "asset/Sound Menu/hard_background_ost.ogg"
        ...
        # Sons d'impact par type de fruit
        self.impact_sounds = {}
        impact_files = {
    ...
    def on_enter(self):
        ...
        try:
            ...
            # On commence à 5 secondes pour plus d'impact
            pygame.mixer.music.play(-1, start=5.0)
        except:
            print("Erreur : Impossible de lire hard_background_ost.ogg")
    ...
    def update(self, dt):
        ...
        if self.lives <= 0:
            if not self.game_over:
                self.game_over = True
                pygame.mixer.music.stop()
                # ENVOI DU SCORE ET DU MODE AU MANAGER POUR LE GAME OVER
                self.manager.shared_data['score'] = self.score.current
                self.manager.shared_data['mode'] = 'hard'
        ...
        self.blade.update(mouse_pos)
        if self.blade.is_moving:
            self.check_cuts()
        else:
            self.blade.fade()
        self.combo_system.update(dt)

        effective_dt = dt * time_mult * HARD_SPEED

        self.spawn_timer += effective_dt
        spawn_interval = get_spawn_interval(self.score.current) * 0.7
    ...
    def render_overlay(self, surface):
        # Overlay effet ice (bleu) ou chili (rouge) avec fade
        if self.freeze_timer > 0:
            opacity = int((self.freeze_timer / 5.0) * 60)
            overlay = pygame.Surface((SCREEN_WIDTH, SCREEN_HEIGHT), pygame.SRCALPHA)
            overlay.fill((0, 100, 255, opacity))
            surface.blit(overlay, (0, 0))
        elif self.frenzy_timer > 0:
            opacity = int((self.frenzy_timer / 5.0) * 60)
            overlay = pygame.Surface((SCREEN_WIDTH, SCREEN_HEIGHT), pygame.SRCALPHA)
            overlay.fill((255, 50, 0, opacity))
            surface.blit(overlay, (0, 0))
        ...
        if self.paused:
            ...
            surface.blit(overlay, (0, 0))
            pause_txt = self.pause_font.render("PAUSE - HARD", True, (255, 255, 255))
    ...
    def spawn_objects(self):
        for _ in range(5):
            ...
            hard_bomb_chance = BOMB_CHANCE * 1.3

            if rand_val < hard_bomb_chance:
                bomb = Bomb3D(x, SCREEN_HEIGHT + 50, model_cache=self.model_cache) if self.use_3d else Bomb(x, SCREEN_HEIGHT + 50)
                self.bombs.append(bomb)
                if self.throw_bomb_sound: self.throw_bomb_sound.play()
            elif rand_val < (hard_bomb_chance + 0.05):
                fruit = Fruit3D(x, SCREEN_HEIGHT + 50, model_cache=self.model_cache) if self.use_3d else Fruit(x, SCREEN_HEIGHT + 50)
                fruit.fruit_type = random.choice(['ice', 'chili'])
    ...
    def check_cuts(self):
        for fruit in self.fruits:
            if not fruit.is_cut and self.blade.collides_with(fruit.x, fruit.y, fruit.radius):
                ...
                self.blade.play_impact_sound()
                impact_sound = self.impact_sounds.get(fruit.fruit_type)
                if impact_sound: impact_sound.play()

        for bomb in self.bombs:
            if not bomb.is_exploded and self.blade.collides_with(bomb.x, bomb.y, bomb.radius):
                bomb.explode()
